...
def calculate_popularity(x):
    budget_places = x[4]
    paid_places = x[5]
    price = manager.get_data('SELECT price FROM po_paid WHERE code=:code AND profile=:profile', {'code': x[0], 'profile': x[2]})[0][0]
...
def hello():
    ...
    direction = [
        {"code": f"{i[0]}", "name": f"{i[2]}", "popularity": f"{int((calculate_popularity(i) / pop_max) * 10)} / 10",
...
def update_data() -> str:
    ...
    if sort == 'popularity':
        ...
        result = sorted(massive_areas, key=lambda x: -(
        manager.get_data('SELECT price FROM po_paid WHERE code=:code AND (profile=:profile OR name=:name)', {'code': x[0], 'profile': x[2], 'name': x[1]})[0][0]))
    elif sort == "budget":
